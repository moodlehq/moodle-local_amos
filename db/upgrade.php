...
function xmldb_local_amos_upgrade($oldversion) {
    ...
    if ($oldversion < 2019090900) {
        ...
        // Define table amos_subscription to be created.
        $table = new xmldb_table('amos_subscription');

        // Adding fields to table amos_subscription.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('component', XMLDB_TYPE_CHAR, '100', null, XMLDB_NOTNULL, null, null);
        $table->add_field('lang', XMLDB_TYPE_CHAR, '20', null, XMLDB_NOTNULL, null, null);

        // Adding keys to table amos_subscription.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('fk_user', XMLDB_KEY_FOREIGN, ['userid'], 'user', ['id']);

        // Conditionally launch create table for amos_subscription.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Amos savepoint reached.
        upgrade_plugin_savepoint(true, 2019090900, 'local', 'amos');
    }
