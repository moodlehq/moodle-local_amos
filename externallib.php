...
/**
 * Provides AMOS functionality to external programs
 */
class local_amos_external extends external_api {
    ...
    public static function update_strings_file_parameters() {
        return new external_function_parameters(
            array(
                'userinfo' => new external_value(PARAM_RAW, 'The name and email of the author to record into AMOS commit'),
                'message' => new external_value(PARAM_RAW, 'The message describing the change to be committed into AMOS repository'),
        ...
        foreach ($components as $component) {
            ...
            if (strpos($component['componentname'], 'mod_') === 0) {
                foreach ($standardplugins as $moodleversion => $frankenstylenames) {
                    if (isset($frankenstylenames[$componentname]) and $frankenstylenames[$componentname] === 'core_'.$componentname) {
                        throw new invalid_parameter_exception('The name of your activity module collides with a name of a core subsystem in Moodle');
                    }
                }
            ...
            $parser->parse($component['stringfilecontent'], $mlangcomponent); // this throws exception if the syntax is wrong
    ...
    public static function update_strings_file_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'componentname' => new external_value(PARAM_COMPONENT, 'The component name as registered in AMOS (eg. stampcoll)'),
                    'moodlebranch' => new external_value(PARAM_FILE, 'The Moodle branch for this component (eg. 2.3)'),
                    'language' => new external_value(PARAM_SAFEDIR, 'The code of the language (eg. en)'),
                    'status' => new external_value(PARAM_ALPHA, 'ok or error or warning'),
                    ...
                    'changes' => new external_value(PARAM_INT, 'Number of committed changes including string removals', VALUE_OPTIONAL),
                )
            )
        );
    }
